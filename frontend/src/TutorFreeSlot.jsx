...
const TutorFreeSlot = () => {
  ...
  // Bạn nhớ thay token thật vào đây hoặc lấy từ LocalStorage
  const AUTH_HEADER =
    "Basic " + btoa("user:93f1df6e-84d4-406d-98e0-a9ffd05e43da");

  // --- HANDLERS ---

  // === 1. HÀM MỚI: KIỂM TRA NGÀY QUÁ KHỨ TRƯỚC KHI MỞ SETUP ===
  const handleOpenSetup = () => {
    // Lấy ngày hôm nay (đặt về 0h00 để so sánh chính xác theo ngày)
    ...
    // So sánh: Nếu ngày chọn nhỏ hơn ngày hiện tại
    if (selected < today) {
      alert("Lỗi: Không thể thiết lập lịch cho ngày trong quá khứ!");
      return; // Dừng lại, không chuyển sang chế độ SETUP
    }

    // Nếu hợp lệ -> Chuyển View
    setViewMode("SETUP");
  };
  // ============================================================
  ...
  const handleSave = () => {
    // (Tuỳ chọn) Kiểm tra lại một lần nữa trước khi lưu
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const selected = new Date(selectedDate);
    selected.setHours(0, 0, 0, 0);
    if (selected < today) {
        alert("Không thể lưu lịch cho ngày quá khứ!");
        return;
    }
        ...
        {viewMode === "VIEW" && (
          <div className="view-mode-container">
            ...
            <div className="calendar-panel">
              {/* === 2. SỬA NÚT BẤM TẠI ĐÂY === */}
              <button
                className="setup-btn"
                onClick={handleOpenSetup} // Gọi hàm kiểm tra thay vì setViewMode trực tiếp
              >
                Thiết lập lịch rảnh
              </button>
