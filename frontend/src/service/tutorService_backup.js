...
export const getPendingAppointments = async (tutorId) => {
  const res = await datacoreApiClient.get(
...
export const approveAppointment = async (appointmentId, tutorId) => {
  const res = await datacoreApiClient.post(
...
export const getSubjects = async () => {
  const res = await datacoreApiClient.get("/api/subjects");
  return res.data;
};

// Get tutors by subject
export const getTutorsBySubject = async (subjectId) => {
  const res = await datacoreApiClient.get(`/api/tutors/by-subject/${subjectId}`);
  return res.data;
};
...
export const getStudentEnrollments = async (bkNetId) => {
  const res = await datacoreApiClient.get(`/api/enrollments/student/${bkNetId}`);
  return res.data;
};
