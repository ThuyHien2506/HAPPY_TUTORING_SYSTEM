...
public class DateValidator implements ScheduleValidator {
    ...
    public void validate(FreeSlotRequest request) {
        // Kiểm tra logic: Ngày chọn không được nhỏ hơn ngày hôm nay
        if (request.getDate().isBefore(LocalDate.now())) {
            throw new IllegalArgumentException("Lỗi: Không thể thao tác với ngày trong quá khứ (" + request.getDate() + ")");
        }
    }
}